                                    ...
                                    int64_t global_timescale)
{
    ...
    *edit_list_duration /= global_timescale;
    //*edit_list_duration = av_rescale(*edit_list_duration, msc->time_scale, global_timescale);
    return 1;
}
...
static void mov_fix_index(MOVContext *mov, AVStream *st)
{
    ...
    while (get_next_edit_list_entry(msc, edit_list_index, &edit_list_media_time,
        ...
        for (; current < e_old_end; current++) {
            ...
            //av_log(mov->fc, AV_LOG_WARNING, "frame_duration = %"PRId64", edit_list_pts_entry_end = %u, edit_list_pts_counter = %u, discard = %d\n", frame_duration, edit_list_pts_entry_end, edit_list_pts_counter, (flags & AVINDEX_DISCARD_FRAME) != 0);
...
static int mov_read_elst(MOVContext *c, AVIOContext *pb, MOVAtom atom)
{
    ...
    av_log(c->fc, AV_LOG_WARNING, "track[%d]\n", c->fc->nb_streams-1);
    ...
    for(i=0; i<edit_count; i++, elst++){
        ...
        av_log(c->fc, AV_LOG_WARNING, "duration=%"PRId64" time=%"PRId64" rate=%f\n",
...
static int mov_read_packet(AVFormatContext *s, AVPacket *pkt)
{
 ...
 retry:
    ...
    if (sc->ctts_data && sc->ctts_index < sc->ctts_count) {
        ...
        /*
         * Hack to make edit list code work correctly: by not setting the duration, the final pts will be set correctly
